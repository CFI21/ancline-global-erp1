...
async function retry(label,fn,attempts=8,delay=1500){let last;for(let i=1;i<=attempts;i++){try{return await fn();}catch(e){last=e;if(i<attempts)await sleep(delay*i);}}throw new Error(label+': '+String(last&&last.message||last));}
const profiles=[
