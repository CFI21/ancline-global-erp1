  ...
  async run(user: any, body: any) {
    ...
    const step = async (
      ...
      try {
        ...
        steps.push({ name, status: 'PASS', durationMs: Date.now() - t0, detail: detail ? detail(value) : undefined });
        return value;
      } catch (e: any) {
    ...
    try {
      const customer = await step('01 Create UAT customer', () => this.p.organization.create({
        data: { code: `${runId}-CUST`, name: `${runId} Customer`, roles: ['CUSTOMER'], countryCode: 'NL' }
      ...
      const rate = await step('03 Create and approve rate', async () => {
        const r = await this.p.rateQuote.create({ data: {
          quoteNo: `${runId}-Q`, customerId: customer.id, trade: 'CNSHA-AEJEA', equipment: '40HC',
          buyRate: 900, sellRate: 1250, currency: 'USD', validFrom: new Date(Date.now() - 86400000),
          validTo: new Date(Date.now() + 30 * 86400000), status: 'VALID', source: 'UAT_RUNNER'
        }});
        return this.p.rateQuote.update({ where: { id: r.id }, data: { status: 'APPROVED' } });
      ...
      await step('06 Clear credit slot equipment controls', () => this.p.booking.update({
        where: { id: booking.id },
        data: { creditStatus: 'Passed', slotStatus: 'Protected', equipmentStatus: 'Available', status: 'CONFIRMED' }
      }), x => ({ status: x.status, credit: x.creditStatus, slot: x.slotStatus, equipment: x.equipmentStatus }));

      const container = await step('07 Add booking container', () => this.p.container.create({ data: {
        containerNo: `UAT${Date.now().toString().slice(-8)}`, bookingId: booking.id, type: '40HC',
        ownership: 'LEASED', status: 'ALLOCATED', location: 'CNSHA'
      }}), x => ({ id: x.id, containerNo: x.containerNo }));
      ids.containerId = container.id;

      const document = await step('08 Create and release booking document', async () => {
        const d = await this.p.document.create({ data: {
          documentNo: `${runId}-BL`, bookingId: booking.id, type: 'SEA_WAYBILL', status: 'Draft', releaseControl: 'Clear'
        }});
        return this.p.document.update({ where: { id: d.id }, data: { status: 'Released' } });
      }, x => ({ id: x.id, documentNo: x.documentNo, status: x.status }));
      ids.documentId = document.id;

      const revenue = await step('09 Create revenue line', () => this.p.financeLine.create({ data: {
        bookingId: booking.id, type: 'REVENUE', chargeCode: 'OCEAN_FREIGHT', amount: 1250,
        finalAmount: 1250, currency: 'USD', status: 'FINAL', source: 'UAT_RUNNER', billingPartyId: customer.id
      }}), x => ({ id: x.id, amount: String(x.amount), status: x.status }));
      ids.revenueId = revenue.id;

      const cost = await step('10 Create cost line', () => this.p.financeLine.create({ data: {
        bookingId: booking.id, type: 'COST', chargeCode: 'SLOT_COST', amount: 900,
        finalAmount: 900, currency: 'USD', status: 'FINAL', source: 'UAT_RUNNER', serviceProviderId: agent.id
      }}), x => ({ id: x.id, amount: String(x.amount), status: x.status }));
      ids.costId = cost.id;

      await step('11 Validate booking P&L', async () => {
        const lines = await this.p.financeLine.findMany({ where: { bookingId: booking.id } });
        const revenueTotal = lines.filter((x: any) => x.type === 'REVENUE').reduce((s: number, x: any) => s + Number(x.finalAmount ?? x.amount), 0);
        const costTotal = lines.filter((x: any) => x.type === 'COST').reduce((s: number, x: any) => s + Number(x.finalAmount ?? x.amount), 0);
      ...
      const task = await step('12 Create and complete task', async () => {
        const t = await this.p.task.create({ data: { bookingId: booking.id, title: `${runId} SI check`, ownerId: actorId, status: 'OPEN' } });
        return this.p.task.update({ where: { id: t.id }, data: { status: 'COMPLETED', slaState: 'Met' } });
      }, x => ({ id: x.id, status: x.status, slaState: x.slaState }));
      ids.taskId = task.id;

      const approval = await step('13 Maker-checker approval', async () => {
        const a = await this.p.approval.create({ data: {
          bookingId: booking.id, type: 'UAT_FINAL_APPROVAL', requesterId: `${actorId}:maker`, approverId: `${actorId}:checker`, status: 'PENDING'
        }});
      ...
      const integration = await step('14 Integration event processing', async () => {
        const e = await this.p.integrationEvent.create({ data: {
          sourceSystem: 'UAT_RUNNER', eventType: 'BOOKING_STATUS', externalId: runId,
          objectType: 'Booking', objectId: booking.id, status: 'RECEIVED', payload: { bookingNo: booking.bookingNo, status: 'CONFIRMED' }
        }});
        return this.p.integrationEvent.update({ where: { id: e.id }, data: { status: 'COMPLETED', attemptCount: 1, completedAt: new Date() } });
      }, x => ({ id: x.id, status: x.status, attemptCount: x.attemptCount }));
      ids.integrationId = integration.id;

      const notification = await step('15 Notification lifecycle', async () => {
        const n = await this.p.notification.create({ data: {
          userId: actorId, category: 'UAT', title: 'UAT booking confirmed', message: booking.bookingNo,
          objectType: 'Booking', objectId: booking.id, status: 'UNREAD'
        }});
        return this.p.notification.update({ where: { id: n.id }, data: { status: 'READ', readAt: new Date() } });
      }, x => ({ id: x.id, status: x.status }));
      ids.notificationId = notification.id;

      await step('16 Audit trail verification', async () => {
        await this.p.auditEvent.createMany({ data: [
          { bookingId: booking.id, actorId, action: 'UAT_BOOKING_CREATE', objectType: 'Booking', objectId: booking.id, detail: { runId } },
          { bookingId: booking.id, actorId, action: 'UAT_FINANCE_VALIDATE', objectType: 'Booking', objectId: booking.id, detail: { gp: 350 } },
          { bookingId: booking.id, actorId, action: 'UAT_APPROVAL_COMPLETE', objectType: 'Approval', objectId: approval.id, detail: { runId } }
        ]});
      ...
      await step('17 Portal visibility data query', async () => {
        const visible = await this.p.booking.findUnique({
          where: { id: booking.id },
          include: { customer: true, producingAgent: true, containers: true, documents: true }
        });
        if (!visible || visible.customerId !== customer.id || visible.containers.length !== 1 || visible.documents.length !== 1) {
      ...
      await step('18 Closeout readiness', async () => {
        ...
        await this.p.jobCloseoutChecklist.createMany({ data: codes.map(itemCode => ({
          bookingId: booking.id, itemCode, itemLabel: itemCode.replace(/_/g, ' '), mandatory: true,
          completed: true, completedBy: actorId, completedAt: new Date(), note: `UAT ${runId}`
        })) });
      ...
      await step('19 Final persistence verification', async () => {
        const final = await this.p.booking.findUnique({ where: { id: booking.id }, include: { financeLines: true, auditEvents: true } });
        if (!final || final.status !== 'FINANCIALLY_CLOSED') throw new Error('Booking did not reach FINANCIALLY_CLOSED');
        return { status: final.status, financeLines: final.financeLines.length, auditEvents: final.auditEvents.length };
      });
