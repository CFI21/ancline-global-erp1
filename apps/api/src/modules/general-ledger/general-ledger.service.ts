...
export class GeneralLedgerService {
  ...
  async setPeriod(body:any,user:ScopeUser){
    this.access(user);const period=this.req(body?.period,'Period');if(!/^\d{4}-\d{2}$/.test(period))throw new BadRequestException('Period must be YYYY-MM');
    ...
    await this.db.integrationEvent.create({data:{sourceSystem:SOURCE,eventType:'PERIOD_STATUS_SET',externalId:period,objectType:'FiscalPeriod',objectId:period,status:'COMPLETED',payload:{period,status,note:body?.note||null,actorId:user.sub},completedAt:new Date()}});
    await this.audit.log({actorId:user.sub,action:'GL_PERIOD_SET',objectType:'FiscalPeriod',objectId:period,detail:{status}});return this.periodState(period);
  }
  ...
  async reopenPeriod(period:string,user:ScopeUser){this.access(user);return this.setPeriod({period,status:'OPEN',note:'Period reopened'},user);}
  ...
  async journals(user:ScopeUser){
    this.access(user);const events:any[]=await this.journalEvents();const grouped=new Map<string,any[]>();
  ...
  async createJournal(body:any,user:ScopeUser){
    this.access(user);const journalDate=this.req(body?.journalDate,'Journal date');const period=this.periodOf(journalDate);const state=await this.periodState(period);if(state.status==='CLOSED')throw new BadRequestException(`Period ${period} is closed`);
