...
export class GroupFinanceService {
  ...
  private resolveEntity(journal:any,bookingMap:Map<string,string|null>){if(journal.entityId)return String(journal.entityId);const explicit=[...new Set((journal.lines||[]).map((l:any)=>l.entityId).filter(Boolean).map(String))];if(explicit.length===1)return explicit[0];if(explicit.length>1)return null;const branches=[...new Set((journal.lines||[]).map((l:any)=>l.bookingId?bookingMap.get(String(l.bookingId)):null).filter(Boolean).map(String))];return branches.length===1?branches[0]:null;}
  ...
  async consolidation(periodInput:string,groupCurrencyInput:string|undefined,user:ScopeUser){
    ...
    const periodJournals=(journals as any[]).filter((j:any)=>j.status==='POSTED'&&j.period===period);const bookingIds=[...new Set(periodJournals.flatMap((j:any)=>(j.lines||[]).map((l:any)=>l.bookingId).filter(Boolean).map(String)))];const bookings:any[]=bookingIds.length?await this.db.booking.findMany({where:{id:{in:bookingIds}},select:{id:true,owningBranchId:true}}):[];const bookingMap=new Map<string,string|null>();for(const b of bookings)bookingMap.set(b.id,b.owningBranchId||null);
