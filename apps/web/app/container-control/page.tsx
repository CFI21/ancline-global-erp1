...
export default function ContainerControlPage(){
  ...
  async function loadBookings(t=token){try{const rows=await api('/bookings',t);const list=Array.isArray(rows)?rows:[];setBookings(list);const contextId=new URLSearchParams(location.search).get('bookingId')||'';if(contextId&&list.some((x:Booking)=>x.id===contextId))void selectBooking(contextId,t);else if(!bookingId&&list[0])void selectBooking(list[0].id,t);}catch(e:any){setMessage(e.message||'Unable to load bookings');}}
  ...
  return <WorkspaceShell title="Container Control" subtitle="Equipment allocation, depot movements, free time, detention / demurrage and event history" active="/container-control" actions={<button className="btn" onClick={()=>bookingId?void selectBooking(bookingId):void loadBookings()}>Refresh</button>}>
    ...
    {bookingId&&<JobFlowNav bookingId={bookingId} bookingNo={selected?.bookingNo} active="CONTAINERS"/>}

    <div className="op-page">
      <div className="op-primary-grid">
        <section className="card op-panel">
          <h3 className="erp-section-title">Booking / Container Scope</h3>
          ...
          {selected&&<div className="op-status-strip" style={{marginTop:7}}><span className="status">{selected.bookingNo}</span><span className="status">{selected.origin} → {selected.destination}</span><span className="status">{containers.length} containers</span><span className="status">{active} active</span><span className="status">{atRisk} at risk</span><span className="status">Exposure {containers[0]?.exposure?.currency||'USD'} {totalExposure.toFixed(2)}</span></div>}
        </section>

        <section className="card op-panel">
          <h3 className="erp-section-title">Record Container Movement</h3>
          <div className="op-field-grid">
          ...
          <div className="op-action-row"><button className="btn" disabled={busy||!form.containerId} onClick={addMovement}>{busy?'Saving…':'Record Movement'}</button></div>
        </section>
      </div>

      {selectedContainer&&<div className="op-detail-grid">
        <details className="op-detail-section">
          <summary>Equipment / Depot / Free Time · {selectedContainer.containerNo}</summary>
        ...
        <details className="op-detail-section">
          <summary>Free Time / Exposure</summary>
          <div className="op-detail-body">
            <div className="op-field-grid">
            ...
            <div className="op-status-strip" style={{marginTop:7}}><span className="status">Demurrage {selectedContainer.exposure?.demurrageDays||0}d · {selectedContainer.exposure?.currency||'USD'} {(selectedContainer.exposure?.demurrageExposure||0).toFixed(2)}</span><span className="status">Detention {selectedContainer.exposure?.detentionDays||0}d · {selectedContainer.exposure?.currency||'USD'} {(selectedContainer.exposure?.detentionExposure||0).toFixed(2)}</span></div>
            <div className="op-action-row"><button className="btn" disabled={busy} onClick={saveControl}>{busy?'Saving…':'Save Equipment Controls'}</button></div>
          </div>
        </details>
      </div>}

    <div className="card op-register-card"><h3 style={sectionTitle}>Container Exposure Register</h3><div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Container</th><th>Allocation</th><th>Provider</th><th>Depot</th><th>Demurrage Free Until</th><th>Detention Free Until</th><th>Empty Return Due</th><th>Exposure</th><th>Risk</th></tr></thead><tbody>
