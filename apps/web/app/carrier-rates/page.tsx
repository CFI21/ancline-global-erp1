...
const blankProvider=()=>({providerCode:'',name:'',carrier:'',carrierOrgId:'',authMode:'BASIC',username:'',secretEnv:'',apiKeyHeader:'x-api-key',endpoint:'',bookingEndpoint:'',carrierBookingNoPath:'',rateBasis:'PER_UNIT',buyIncludesSurcharges:false,defaultPricingMethod:'MARKUP_PCT',defaultPricingValue:'15',minimumMarkupPct:'0',paymentTermsDays:'30',paymentMethod:'BANK_TRANSFER',prepaidPct:'0',creditLimit:'',creditCurrency:'USD',responseArrayPath:'',buyRatePath:'',currencyPath:'',surchargesPath:'',surchargeChargeCodePath:'',surchargeAmountPath:'',externalQuoteRefPath:'',notes:''});
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Provider</th><th>Connection</th><th>Rate Basis</th><th>Pricing Control</th><th>Payment Terms</th><th>Credit</th><th>Status</th></tr></thead>
          <tbody>{!ctx?.providers?.length?<tr><td colSpan={7}>No online carrier accounts configured yet. Filed service-contract rates are still searched.</td></tr>:ctx.providers.map(p=><tr key={p.providerCode}>
            ...
            <td>{p.endpoint?'RATE API SET':'NO RATE API'}<div className="sub">{p.bookingEndpoint?'Booking API set':'Booking API missing'} · {p.authMode==='NONE'?'No secret required':p.secretConfigured?'Secret configured':'Secret missing'}</div></td>
