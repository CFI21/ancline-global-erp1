...
const blankProvider=()=>({providerCode:'',name:'',carrier:'',carrierOrgId:'',authMode:'BASIC',username:'',secretEnv:'',apiKeyHeader:'x-api-key',endpoint:'',bookingEndpoint:'',amendmentEndpoint:'',cancellationEndpoint:'',vgmEndpoint:'',shippingInstructionsEndpoint:'',blDraftEndpoint:'',trackingEndpoint:'',carrierBookingNoPath:'',carrierAccountName:'ANC',carrierAccountCode:'',masterShipperName:'ANC',masterConsigneeName:'',masterNotifyPartyName:'',capabilities:{RATES:'API',BOOKING:'API',AMENDMENT:'DISABLED',CANCELLATION:'DISABLED',VGM:'DISABLED',SHIPPING_INSTRUCTIONS:'DISABLED',BL_DRAFT:'DISABLED',TRACKING:'DISABLED'},optionalCapabilities:{ADDITIONAL_FREE_TIME:'DISABLED',GREEN_PRODUCT:'DISABLED',SHIPPING_GUARANTEE:'DISABLED',LIVE_REEFER:'DISABLED',INLAND_RATE:'DISABLED',LOCAL_CHARGES:'DISABLED'},rateBasis:'PER_UNIT',buyIncludesSurcharges:false,defaultPricingMethod:'MARKUP_PCT',defaultPricingValue:'15',minimumMarkupPct:'0',paymentTermsDays:'30',paymentMethod:'BANK_TRANSFER',prepaidPct:'0',creditLimit:'',creditCurrency:'USD',responseArrayPath:'',buyRatePath:'',currencyPath:'',surchargesPath:'',surchargeChargeCodePath:'',surchargeAmountPath:'',externalQuoteRefPath:'',notes:''});
    ...
    {booking&&<>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Provider</th><th>Connection</th><th>Rate Basis</th><th>Pricing Control</th><th>Payment Terms</th><th>Credit</th><th>Status</th></tr></thead>
          <tbody>{!ctx?.providers?.length?<tr><td colSpan={7}>No online carrier accounts configured yet. Filed service-contract rates are still searched.</td></tr>:ctx.providers.map(p=><tr key={p.providerCode}>
            ...
            <td>{p.endpoint?'RATE API SET':'NO RATE API'}<div className="sub">{p.bookingEndpoint?'Booking API set':'Booking API missing'} · {p.authMode==='NONE'?'No secret required':p.secretConfigured?'Secret configured':'Secret missing'}</div><div className="sub">Identity shield: STRICT · ANC carrier account only</div><div className="sub">Core: {Object.entries((p as any).capabilities||{}).filter(([,v])=>v!=='DISABLED').map(([k,v])=>k+'='+v).join(' · ')||'None enabled'}</div></td>
      ...
      <div className="card">
        ...
        <div style={formGrid}>
          ...
          <div style={{gridColumn:'1/-1',marginTop:6}}><h4 style={{margin:'4px 0'}}>ANC Carrier Identity Shield</h4><div className="sub">Carrier-facing identity only. Customer KYC, customer references and HBL parties are never sent through normal carrier integrations.</div></div>
          <label><span style={labelStyle}>ANC Carrier Account Name</span><input style={fieldStyle} value={provider.carrierAccountName} onChange={e=>setProvider({...provider,carrierAccountName:e.target.value})} placeholder="ANC / ANC legal entity"/></label>
          <label><span style={labelStyle}>ANC Carrier Account Code</span><input style={fieldStyle} value={provider.carrierAccountCode} onChange={e=>setProvider({...provider,carrierAccountCode:e.target.value})} placeholder="Carrier-assigned ANC account"/></label>
          <label><span style={labelStyle}>MBL Shipper</span><input style={fieldStyle} value={provider.masterShipperName} onChange={e=>setProvider({...provider,masterShipperName:e.target.value})} placeholder="ANC legal entity"/></label>
          <label><span style={labelStyle}>MBL Consignee / Destination Agent</span><input style={fieldStyle} value={provider.masterConsigneeName} onChange={e=>setProvider({...provider,masterConsigneeName:e.target.value})} placeholder="ANC branch / approved agent"/></label>
          <label><span style={labelStyle}>MBL Notify Party</span><input style={fieldStyle} value={provider.masterNotifyPartyName} onChange={e=>setProvider({...provider,masterNotifyPartyName:e.target.value})} placeholder="ANC / approved agent"/></label>
          ...
          <label><span style={labelStyle}>HTTPS Rate Endpoint</span><input style={fieldStyle} value={provider.endpoint} onChange={e=>setProvider({...provider,endpoint:e.target.value})} placeholder="https://carrier-api.example/rates"/></label>\n          <label><span style={labelStyle}>HTTPS Booking Endpoint</span><input style={fieldStyle} value={provider.bookingEndpoint} onChange={e=>setProvider({...provider,bookingEndpoint:e.target.value})} placeholder="https://carrier-api.example/bookings"/></label>
          <div style={{gridColumn:'1/-1',marginTop:6}}><h4 style={{margin:'4px 0'}}>Core Automation Capabilities</h4><div className="sub">Only enable a capability when the carrier agreement / API / EDI channel actually supports it.</div></div>
          <label><span style={labelStyle}>RATES</span><select style={fieldStyle} value={provider.capabilities?.RATES||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,RATES:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>BOOKING</span><select style={fieldStyle} value={provider.capabilities?.BOOKING||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,BOOKING:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>AMENDMENT</span><select style={fieldStyle} value={provider.capabilities?.AMENDMENT||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,AMENDMENT:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>CANCELLATION</span><select style={fieldStyle} value={provider.capabilities?.CANCELLATION||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,CANCELLATION:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>VGM</span><select style={fieldStyle} value={provider.capabilities?.VGM||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,VGM:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>SHIPPING INSTRUCTIONS</span><select style={fieldStyle} value={provider.capabilities?.SHIPPING_INSTRUCTIONS||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,SHIPPING_INSTRUCTIONS:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>BL DRAFT</span><select style={fieldStyle} value={provider.capabilities?.BL_DRAFT||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,BL_DRAFT:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>TRACKING</span><select style={fieldStyle} value={provider.capabilities?.TRACKING||'DISABLED'} onChange={e=>setProvider({...provider,capabilities:{...provider.capabilities,TRACKING:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>Amendment API Endpoint</span><input style={fieldStyle} value={provider.amendmentEndpoint} onChange={e=>setProvider({...provider,amendmentEndpoint:e.target.value})} placeholder="https://carrier-api.example/amendments"/></label>
          <label><span style={labelStyle}>Cancellation API Endpoint</span><input style={fieldStyle} value={provider.cancellationEndpoint} onChange={e=>setProvider({...provider,cancellationEndpoint:e.target.value})} placeholder="https://carrier-api.example/cancellations"/></label>
          <label><span style={labelStyle}>VGM API Endpoint</span><input style={fieldStyle} value={provider.vgmEndpoint} onChange={e=>setProvider({...provider,vgmEndpoint:e.target.value})}/></label>
          <label><span style={labelStyle}>Shipping Instructions API Endpoint</span><input style={fieldStyle} value={provider.shippingInstructionsEndpoint} onChange={e=>setProvider({...provider,shippingInstructionsEndpoint:e.target.value})}/></label>
          <label><span style={labelStyle}>B/L Draft API Endpoint</span><input style={fieldStyle} value={provider.blDraftEndpoint} onChange={e=>setProvider({...provider,blDraftEndpoint:e.target.value})}/></label>
          <label><span style={labelStyle}>Tracking API Endpoint</span><input style={fieldStyle} value={provider.trackingEndpoint} onChange={e=>setProvider({...provider,trackingEndpoint:e.target.value})}/></label>
          <div style={{gridColumn:'1/-1',marginTop:6}}><h4 style={{margin:'4px 0'}}>Optional Future Carrier Products</h4><div className="sub">Not part of ANC core business. Keep DISABLED unless ANC later has a commercial agreement for that product.</div></div>
          <label><span style={labelStyle}>ADDITIONAL FREE TIME</span><select style={fieldStyle} value={provider.optionalCapabilities?.ADDITIONAL_FREE_TIME||'DISABLED'} onChange={e=>setProvider({...provider,optionalCapabilities:{...provider.optionalCapabilities,ADDITIONAL_FREE_TIME:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>GREEN PRODUCT</span><select style={fieldStyle} value={provider.optionalCapabilities?.GREEN_PRODUCT||'DISABLED'} onChange={e=>setProvider({...provider,optionalCapabilities:{...provider.optionalCapabilities,GREEN_PRODUCT:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>SHIPPING GUARANTEE</span><select style={fieldStyle} value={provider.optionalCapabilities?.SHIPPING_GUARANTEE||'DISABLED'} onChange={e=>setProvider({...provider,optionalCapabilities:{...provider.optionalCapabilities,SHIPPING_GUARANTEE:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>LIVE REEFER</span><select style={fieldStyle} value={provider.optionalCapabilities?.LIVE_REEFER||'DISABLED'} onChange={e=>setProvider({...provider,optionalCapabilities:{...provider.optionalCapabilities,LIVE_REEFER:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>INLAND RATE</span><select style={fieldStyle} value={provider.optionalCapabilities?.INLAND_RATE||'DISABLED'} onChange={e=>setProvider({...provider,optionalCapabilities:{...provider.optionalCapabilities,INLAND_RATE:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
          <label><span style={labelStyle}>LOCAL CHARGES</span><select style={fieldStyle} value={provider.optionalCapabilities?.LOCAL_CHARGES||'DISABLED'} onChange={e=>setProvider({...provider,optionalCapabilities:{...provider.optionalCapabilities,LOCAL_CHARGES:e.target.value}})}><option>DISABLED</option><option>API</option><option>EDI</option><option>MANUAL</option></select></label>
\n          <label><span style={labelStyle}>Carrier Booking Ref. Path</span><input style={fieldStyle} value={provider.carrierBookingNoPath} onChange={e=>setProvider({...provider,carrierBookingNoPath:e.target.value})} placeholder="data.bookingReference"/></label>
