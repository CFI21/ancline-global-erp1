...
export default function FinancePage(){
  ...
  return <WorkspaceShell title="Finance / Job Costing" subtitle="Commercial handover, revenue, cost, margin, accruals, invoicing readiness and financial close" active="/finance" actions={<button className="btn" onClick={()=>void loadFinance()}>Refresh</button>}>
    ...
    <div className="card" style={{marginBottom:12}}><h3 style={sectionTitle}>Add Job Charge</h3><div style={formGrid}>
      ...
      <label><span style={labelStyle}>Currency</span><select className="erp-field-code" style={fieldStyle} value={form.currency} onChange={e=>setForm({...form,currency:e.target.value})}>{['USD','EUR','GBP','AED'].map(x=><option key={x}>{x}</option>)}</select></label>
      <label><span style={labelStyle}>Tax %</span><input className="erp-field-short" type="number" step="0.01" style={fieldStyle} value={form.taxRate} onChange={e=>setForm({...form,taxRate:e.target.value})}/></label>
