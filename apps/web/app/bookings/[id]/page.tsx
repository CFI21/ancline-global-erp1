...
type Booking={
  id:string;bookingNo:string;customerId:string;producingAgentId?:string|null;bookingType?:string|null;transportMode?:string|null;serviceType?:string|null;bookingDate?:string|null;
  ...
  incoterm?:string|null;freightTerms?:string|null;currency:string;specialCargo?:string|null;notes?:string|null;status:string;creditStatus?:string|null;slotStatus?:string|null;equipmentStatus?:string|null;
  customer?:{name:string};producingAgent?:{name:string};containers:Container[];documents:any[];financeLines:any[];tasks:any[];approvals:any[];auditEvents:AuditEvent[];
};

const blank={
  bookingNo:'',customerId:'',producingAgentId:'',bookingType:'FCL',transportMode:'SEA',serviceType:'CY/CY',bookingDate:'',customerReference:'',shipperReference:'',carrierBookingNo:'',houseBL:'',masterBL:'',
  ...
  incoterm:'',freightTerms:'PREPAID',currency:'USD',specialCargo:'',notes:'',creditStatus:'',slotStatus:'',equipmentStatus:''
};
...
export default function EditBookingPage(){
  ...
  useEffect(()=>{const t=localStorage.getItem('ancline_token')||'';if(!t){location.href='/login';return;}setToken(t);void load(t);},[id]);
  ...
  async function load(t=token){
    ...
    try{
      ...
      setForm({
        ...
        incoterm:b.incoterm||'',freightTerms:b.freightTerms||'PREPAID',currency:b.currency||'USD',specialCargo:b.specialCargo||'',notes:b.notes||'',creditStatus:b.creditStatus||'',slotStatus:b.slotStatus||'',equipmentStatus:b.equipmentStatus||''
      });
  ...
  async function save(close=false){
    ...
    try{
      await req(`/bookings/${id}`,{method:'PATCH',body:JSON.stringify({
        ...form,producingAgentId:form.producingAgentId||null,bookingDate:toIso(form.bookingDate),etd:toIso(form.etd),eta:toIso(form.eta),atd:toIso(form.atd),ata:toIso(form.ata),cyClosing:toIso(form.cyClosing),siCutoff:toIso(form.siCutoff),vgmCutoff:toIso(form.vgmCutoff),docCutoff:toIso(form.docCutoff),portCutoff:toIso(form.portCutoff),
        quantity:num(form.quantity),packageCount:num(form.packageCount),grossWeight:num(form.grossWeight),netWeight:num(form.netWeight),volumeCbm:num(form.volumeCbm)
      })});
      if(close){location.href='/bookings';return;}
  ...
  async function advance(){setBusy(true);setMessage('');try{await req(`/bookings/${id}/advance`,{method:'POST'});setMessage('Booking workflow advanced.');await load();}catch(e:any){setMessage(e?.message||'Could not advance workflow');}finally{setBusy(false);}}

  async function addContainer(){
    if(!container.containerNo.trim()){setMessage('Enter a container number first.');return;}
    setBusy(true);setMessage('');
    try{await req(`/bookings/${id}/containers`,{method:'POST',body:JSON.stringify({...container,vgm:num(container.vgm),grossWeight:num(container.grossWeight),pickupDate:container.pickupDate?new Date(`${container.pickupDate}T00:00:00Z`).toISOString():null})});setContainer({containerNo:'',type:'40HC',ownership:'CARRIER',status:'PLANNED',location:'',sealNo:'',vgm:'',grossWeight:'',pickupDate:'',emptyDepot:'',fullReturnTerminal:''});setMessage('Container added.');await load();}catch(e:any){setMessage(e?.message||'Could not add container');}finally{setBusy(false);}
  }
  ...
  return <div className="shell"><aside className="side"><div className="brand">ANCLINE WORLDWIDE</div><a href="/">Control Tower</a><a href="/bookings" style={{background:'#183a5c'}}>Bookings</a><a href="/organizations">Organizations</a><a href="/rates">Rates / Quotes</a><a href="/documents">Documents</a><a href="/finance">Finance</a><a href="/approvals">Approvals</a><a href="/tasks">My Work</a></aside><main className="main">
    <div className="top"><div><div className="sub">BOOKING WORKSPACE</div><h1 style={{margin:'2px 0'}}>{booking.bookingNo}</h1><div style={{display:'flex',gap:7,flexWrap:'wrap'}}><span className="status">{booking.status}</span><span className="status">{booking.customer?.name||'No customer'}</span><span className="status">{booking.origin} → {booking.destination}</span><span className="status">ETD {booking.etd?new Date(booking.etd).toLocaleDateString():'-'}</span><span className="status">Containers {booking.containers?.length||0}</span></div></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><a className="btn" href="/bookings" style={{textDecoration:'none'}}>← Register</a><button className="btn" onClick={()=>save(false)} disabled={busy}>{busy?'Working...':'Save Changes'}</button><button className="btn" onClick={()=>save(true)} disabled={busy}>Save & Close</button><button className="btn" onClick={advance} disabled={busy}>Advance Status</button><button className="btn" onClick={signOut}>Sign out</button></div></div>
    {message&&<div className="card" style={{marginBottom:12}}>{message}</div>}

    <div className="card" style={{marginBottom:12}}><h3 style={title}>Booking Details & References</h3><div style={grid}><Input l="Booking No." k="bookingNo"/><Input l="Booking Date" k="bookingDate" type="date"/><Input l="Carrier Booking No." k="carrierBookingNo"/><Input l="Customer Ref." k="customerReference"/><Input l="Shipper Ref." k="shipperReference"/><Input l="House B/L" k="houseBL"/><Input l="Master B/L" k="masterBL"/><Select l="Booking Type" k="bookingType"><option>FCL</option><option>LCL</option><option>BREAKBULK</option><option>RORO</option></Select><Select l="Transport Mode" k="transportMode"><option>SEA</option><option>AIR</option><option>ROAD</option><option>RAIL</option></Select><Select l="Service Type" k="serviceType"><option>CY/CY</option><option>DOOR/CY</option><option>CY/DOOR</option><option>DOOR/DOOR</option></Select><Select l="Freight Terms" k="freightTerms"><option>PREPAID</option><option>COLLECT</option></Select><Select l="Currency" k="currency"><option>USD</option><option>EUR</option><option>GBP</option><option>AED</option></Select><Input l="Incoterm" k="incoterm"/></div></div>
    ...
    <div className="card" style={{marginBottom:12}}><h3 style={title}>Routing & Agents</h3><div style={grid}><Input l="Place of Receipt" k="placeOfReceipt"/><Input l="Origin" k="origin"/><Input l="Port of Loading (POL)" k="portOfLoading"/><Input l="POL Agent" k="polAgent"/><Input l="Transshipment Port" k="transshipmentPort"/><Input l="Port of Discharge (POD)" k="portOfDischarge"/><Input l="POD Agent" k="podAgent"/><Input l="Destination" k="destination"/><Input l="Place of Delivery" k="placeOfDelivery"/><Input l="Terminal" k="terminal"/></div></div>
    ...
    <div className="card" style={{marginBottom:12}}><h3 style={title}>Equipment & Cargo</h3><div style={grid}><Select l="Equipment" k="equipment"><option>20GP</option><option>40GP</option><option>40HC</option><option>45HC</option><option>20RF</option><option>40RF</option><option>20OT</option><option>40OT</option><option>20FR</option><option>40FR</option></Select><Input l="Quantity" k="quantity" type="number"/><Select l="Container Owner" k="containerOwner"><option>CARRIER</option><option>SHIPPER</option><option>ANCLINE</option><option>SOC</option></Select><Input l="Commodity" k="commodity"/><Input l="Packages" k="packageCount" type="number"/><Input l="Package Type" k="packageType"/><Input l="Gross Weight (kg)" k="grossWeight" type="number"/><Input l="Net Weight (kg)" k="netWeight" type="number"/><Input l="Volume (CBM)" k="volumeCbm" type="number"/><Input l="HS Code" k="hsCode"/><Input l="Special Cargo" k="specialCargo"/><Input l="Marks & Numbers" k="marksNumbers"/><Input l="Cargo Description" k="cargoDescription"/></div></div>

    <div className="card" style={{marginBottom:12}}><h3 style={title}>Operational Controls</h3><div style={grid}><Select l="Credit Status" k="creditStatus"><option value="">Not checked</option><option>Pending</option><option>Passed</option><option>Blocked</option></Select><Select l="Slot Status" k="slotStatus"><option value="">Not checked</option><option>Pending</option><option>Protected</option><option>Waitlist</option></Select><Select l="Equipment Status" k="equipmentStatus"><option value="">Not checked</option><option>Pending</option><option>Available</option><option>Shortage</option></Select></div></div>

    <div className="card" style={{marginBottom:12}}><h3 style={title}>Containers</h3><div style={{...grid,marginBottom:12}}>
      ...
      <label><span style={label}>Owner</span><select value={container.ownership} onChange={e=>setContainer(v=>({...v,ownership:e.target.value}))} style={field}><option>CARRIER</option><option>SHIPPER</option><option>ANCLINE</option><option>SOC</option></select></label>
      <label><span style={label}>Seal No.</span><input value={container.sealNo} onChange={e=>setContainer(v=>({...v,sealNo:e.target.value}))} style={field}/></label>
      <label><span style={label}>Gross Weight (kg)</span><input type="number" value={container.grossWeight} onChange={e=>setContainer(v=>({...v,grossWeight:e.target.value}))} style={field}/></label>
      <label><span style={label}>VGM (kg)</span><input type="number" value={container.vgm} onChange={e=>setContainer(v=>({...v,vgm:e.target.value}))} style={field}/></label>
      <label><span style={label}>Pickup Date</span><input type="date" value={container.pickupDate} onChange={e=>setContainer(v=>({...v,pickupDate:e.target.value}))} style={field}/></label>
      <label><span style={label}>Empty Depot</span><input value={container.emptyDepot} onChange={e=>setContainer(v=>({...v,emptyDepot:e.target.value}))} style={field}/></label>
      <label><span style={label}>Full Return Terminal</span><input value={container.fullReturnTerminal} onChange={e=>setContainer(v=>({...v,fullReturnTerminal:e.target.value}))} style={field}/></label>
      <label><span style={label}>Status</span><select value={container.status} onChange={e=>setContainer(v=>({...v,status:e.target.value}))} style={field}><option>PLANNED</option><option>RELEASED</option><option>PICKED_UP</option><option>GATED_IN</option><option>LOADED</option><option>DISCHARGED</option><option>DELIVERED</option><option>EMPTY_RETURNED</option></select></label>
    ...
    </div><div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Container</th><th>Type</th><th>Owner</th><th>Seal</th><th>Gross kg</th><th>VGM kg</th><th>Pickup</th><th>Depot</th><th>Return Terminal</th><th>Status</th><th>Location</th><th></th></tr></thead><tbody>{!booking.containers?.length?<tr><td colSpan={12}>No containers assigned.</td></tr>:booking.containers.map(c=><tr key={c.id}><td><b>{c.containerNo}</b></td><td>{c.type}</td><td>{c.ownership}</td><td>{c.sealNo||'-'}</td><td>{c.grossWeight??'-'}</td><td>{c.vgm??'-'}</td><td>{c.pickupDate?new Date(c.pickupDate).toLocaleDateString():'-'}</td><td>{c.emptyDepot||'-'}</td><td>{c.fullReturnTerminal||'-'}</td><td>{c.status}</td><td>{c.location||'-'}</td><td><button className="btn" onClick={()=>removeContainer(c.id)} disabled={busy}>Remove</button></td></tr>)}</tbody></table></div></div>

    <div className="card" style={{marginBottom:12}}><h3 style={title}>Operational Notes</h3><textarea value={form.notes} onChange={e=>set('notes',e.target.value)} style={{...field,minHeight:90,resize:'vertical'}}/></div>

    <div className="card"><h3 style={title}>Recent Activity</h3><div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Date</th><th>Action</th><th>Actor</th></tr></thead><tbody>{!booking.auditEvents?.length?<tr><td colSpan={3}>No audit activity yet.</td></tr>:booking.auditEvents.slice().sort((a,b)=>new Date(b.createdAt).getTime()-new Date(a.createdAt).getTime()).slice(0,10).map(a=><tr key={a.id}><td>{new Date(a.createdAt).toLocaleString()}</td><td>{a.action}</td><td>{a.actorId}</td></tr>)}</tbody></table></div></div>
  </main></div>;
}
