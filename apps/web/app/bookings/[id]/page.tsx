...
const tabs=['Details','Routing','Containers','Cargo','Documents','Charges','Tasks','Approvals','Audit'] as const;
...
export default function EditBookingPage(){
  ...
  async function save(close=false){
    if(!form.origin.trim()||!form.destination.trim()){setMessage('Origin and Destination are required.');return;}const cargoError=validateCargo();if(cargoError){setMessage(cargoError);setTab('Cargo');return;}
    setBusy(true);setMessage('');
    try{await req(`/bookings/${id}`,{method:'PATCH',body:JSON.stringify(bookingPayload())});if(close){location.href='/bookings';return;}setMessage('Booking changes saved successfully.');await load();}
