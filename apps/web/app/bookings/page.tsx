...
export default function BookingsPage(){
  ...
  >
    ...
    <div className="card"><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginBottom:10,flexWrap:'wrap'}}><div><h3 style={{margin:0}}>Booking Register</h3><span className="sub">Search, filter, export and open bookings for operations.</span></div><div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><button className="btn" onClick={clearFilters}>Clear Filters</button><button className="btn" onClick={exportCsv} disabled={!visible.length}>Export CSV</button><span className="status">{visible.length} bookings</span></div></div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:8,marginBottom:12}}><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search bookings..." style={field}/><select aria-label="Filter bookings by status" value={statusFilter} onChange={e=>setStatusFilter(e.target.value)} style={field}><option value="ALL">All statuses</option>{statuses.map(s=><option key={s}>{s}</option>)}</select><select aria-label="Filter bookings by customer" value={customerFilter} onChange={e=>setCustomerFilter(e.target.value)} style={field}><option value="ALL">All customers</option>{customerNames.map(s=><option key={s}>{s}</option>)}</select><select aria-label="Filter bookings by carrier" value={carrierFilter} onChange={e=>setCarrierFilter(e.target.value)} style={field}><option value="ALL">All carriers</option>{carriers.map(s=><option key={s}>{s}</option>)}</select><select aria-label="Filter bookings by attention state" value={attentionFilter} onChange={e=>setAttentionFilter(e.target.value)} style={field}><option value="ALL">All attention states</option><option value="ACTION">Needs action</option><option value="CUTOFF">Cutoff risk</option><option value="CONTROLS">Control gaps</option><option value="SPECIAL">Special cargo</option></select><label><span style={label}>ETD From</span><input type="date" value={etdFrom} onChange={e=>setEtdFrom(e.target.value)} style={field}/></label><label><span style={label}>ETD To</span><input type="date" value={etdTo} onChange={e=>setEtdTo(e.target.value)} style={field}/></label></div>
