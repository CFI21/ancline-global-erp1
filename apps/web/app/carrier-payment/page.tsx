      ...
      <div className="card">
        ...
        <div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Charge Group</th><th>Payment Term</th><th>ANC Payer</th><th>Elsewhere Name / Address</th><th>Country</th><th>Carrier Payer Code</th></tr></thead><tbody>
          {GROUPS.map(([g,label])=>{const row=rows[g]||blank(false);const elsewhere=row.term==='PREPAID_ELSEWHERE',na=row.term==='NOT_APPLICABLE';return <tr key={g}>
            ...
            <td>{!na?<select style={{...fieldStyle,minWidth:190}} value={row.payerOfficeKey} onChange={e=>{const key=e.target.value,o=offices.find(x=>x.officeKey===key);set(g,'payerOfficeKey',key);if(elsewhere&&o){set(g,'payerCountryCode',o.countryCode);}}}><option value="">Select ANC office</option>{offices.map(o=><option key={o.officeKey} value={o.officeKey}>{o.code} · {o.name} · {o.countryCode}</option>)}</select>:<span className="sub">Not applicable</span>}</td>
