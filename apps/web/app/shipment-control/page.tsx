...
export default function ShipmentControlPage(){
  ...
  return <WorkspaceShell title="Shipment / Consol Control" subtitle="Promote bookings into operational shipments and manage master consol movements" active="/shipment-control" actions={<><button className="btn" onClick={()=>setShowShipmentForm(v=>!v)}>+ Shipment</button><button className="btn" onClick={()=>setShowConsolForm(v=>!v)}>+ Consol</button></>}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:12,marginBottom:12}}>
      <Kpi l="House Shipments" v={dashboard.houseShipments}/><Kpi l="Unconsolidated" v={dashboard.unconsolidated}/><Kpi l="Active Consols" v={dashboard.activeConsols}/><Kpi l="In Transit" v={dashboard.inTransit}/><Kpi l="Arrived" v={dashboard.arrived}/>
    </div>
    ...
    {showConsolForm&&<div className="card" style={{marginBottom:12}}><h3 style={sectionTitle}>Create Master Consol</h3><div style={formGrid}>
    ...
    <div className="card"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}><div><h3 style={{margin:0}}>Master Consol Board</h3><span className="sub">{consols.length} consol movements</span></div></div>
      <div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Consol / MBL</th><th>Carrier / Vessel</th><th>Route</th><th>Schedule</th><th>Shipments</th><th>Status</th><th>Action</th></tr></thead><tbody>{consols.length===0?<tr><td colSpan={7}>No consols created.</td></tr>:consols.map(c=><tr key={c.id}><td><b>{c.consolNo}</b><div className="sub">{c.masterBL||'No MBL assigned'}</div></td><td>{c.carrier||'-'}<div className="sub">{c.vessel||'-'} {c.voyage?`/ ${c.voyage}`:''}</div></td><td>{c.portOfLoading} → {c.portOfDischarge}<div className="sub">{c.origin} → {c.destination}</div></td><td>ETD {fmtDate(c.etd)}<div className="sub">ETA {fmtDate(c.eta)}</div>{c.atd&&<div className="sub">ATD {fmtDate(c.atd)}</div>}{c.ata&&<div className="sub">ATA {fmtDate(c.ata)}</div>}</td><td><b>{c.shipments.length}</b><div className="sub">{c.shipments.slice(0,3).map(s=>s.shipmentNo||s.bookingNo).join(', ')}{c.shipments.length>3?'…':''}</div></td><td><span className="status">{c.status}</span></td><td><div style={{display:'flex',gap:5,flexWrap:'wrap',minWidth:190}}>{c.status==='PLANNED'&&<button className="btn" disabled={busy} onClick={()=>action(c.id,'confirm')}>Confirm</button>}{c.status==='CONFIRMED'&&<button className="btn" disabled={busy} onClick={()=>action(c.id,'depart')}>Depart</button>}{c.status==='DEPARTED'&&<button className="btn" disabled={busy} onClick={()=>action(c.id,'arrive')}>Arrive</button>}{c.status==='ARRIVED'&&<button className="btn" disabled={busy} onClick={()=>action(c.id,'close')}>Close</button>}</div></td></tr>)}</tbody></table></div>
    </div>
  </WorkspaceShell>;
}
