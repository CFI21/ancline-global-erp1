...
export default function BulkDataPage(){
  ...
  async function validate(){
    ...
    try{const r=await api('/bulk-data/validate',token,{method:'POST',body:JSON.stringify({type:kind,rows})});setValidation(r);setMessage(r.invalid?('Validation found '+r.invalid+' invalid rows.'):'Validation passed for all '+r.valid+' rows.');}
  ...
  async function importRows(){
    ...
    try{const r=await api('/bulk-data/import',token,{method:'POST',body:JSON.stringify({type:kind,rows})});setResult(r);setValidation(null);setMessage(r.committed?('Bulk import '+r.batchId+': '+r.succeeded+' succeeded, '+r.failed+' failed.'):(r.message||'Import was not committed.'));await load();}
  ...
  return <WorkspaceShell title="Bulk Data Upload" subtitle="Customer, carrier, rate, booking and controlled synthetic test-data loading" active="/bulk-data"
    ...
    <div className="card" style={{marginBottom:12}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginBottom:10}}><span className="status">{rows.length} rows loaded</span><button className="btn" disabled={busy||!rows.length||!admin} onClick={validate}>Validate</button><button className="btn" disabled={busy||!rows.length||!admin} onClick={importRows}>Import / Upsert</button></div>
    ...
    {validation&&<div className="card" style={{marginBottom:12}}><h3 style={sectionTitle}>Validation Result</h3>
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}><span className="status">Valid {validation.valid}</span><span className="status">Invalid {validation.invalid}</span></div>
    ...
    {result&&<div className="card" style={{marginBottom:12}}><h3 style={sectionTitle}>Import Result {result.batchId||''}</h3>
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}><span className="status">Succeeded {result.succeeded||0}</span><span className="status">Failed {result.failed||0}</span><span className="status">Committed {String(Boolean(result.committed))}</span></div>
