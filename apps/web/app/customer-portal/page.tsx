...
export default function CustomerPortal(){
  ...
  const [loading,setLoading]=useState(true);const user=currentUser();
  ...
  return <main style={{padding:18,maxWidth:1280,margin:'0 auto'}}>
    <div className="top"><div><div className="sub">CUSTOMER PORTAL</div><h1 style={{margin:'2px 0'}}>My Shipments</h1><div className="sub">Live operational visibility · {user?.email||''}</div></div><div style={{display:'flex',gap:8}}><button className="btn" onClick={()=>location.reload()}>Refresh</button><button className="btn" onClick={signOut}>Sign out</button></div></div>
    {err&&<div className="card" style={{marginBottom:12}}>Portal notice: {err}</div>}
    <PortalRateBooking token={requireToken()||''} role="CUSTOMER" onBooked={()=>location.reload()}/>
