...
type User={id:string;email:string;displayName:string;role:string;branchId?:string|null;agentId?:string|null;customerId?:string|null;partyId?:string|null;active:boolean};
...
const emptyUser={email:'',displayName:'',role:'BRANCH_OPS',branchId:'',agentId:'',customerId:'',partyId:''};
...
export default function AdministrationPage(){
  ...
  function editUser(row:User){setEditingId(row.id);setUser({email:row.email,displayName:row.displayName,role:row.role,branchId:row.branchId||'',agentId:row.agentId||'',customerId:row.customerId||'',partyId:row.partyId||''});setTab('USERS');window.scrollTo({top:0,behavior:'smooth'});}
  ...
  const roleChanged=(role:string)=>setUser(x=>({...x,role,branchId:role==='BRANCH_OPS'||role==='FINANCE'?x.branchId:'',agentId:role==='AGENT'?x.agentId:'',customerId:role==='CUSTOMER'?x.customerId:'',partyId:role==='SHIPPER'||role==='CONSIGNEE'?x.partyId:''}));
  ...
  return <WorkspaceShell title="Administration" subtitle="Branches, managed user access, scopes and integration monitoring" active="/administration" actions={<button className="btn" onClick={()=>void load()}>Refresh</button>}>
    ...
    {tab==='USERS'&&<>
      ...
      </div><div className="sub" style={{marginTop:10}}>Provisioned users receive their role and scope from this record at login. Customer, shipper, consignee and agent users cannot choose another organization from the login screen once provisioned.</div><div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:12}}>{editingId&&<button className="btn" onClick={resetUser}>Cancel</button>}<button className="btn" disabled={busy} onClick={saveUser}>{busy?'Saving...':editingId?'Save Access':'Create User'}</button></div></div>
