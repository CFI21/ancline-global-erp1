...
const ACCESS_RIGHTS=['NVOCC_PORTAL','NVOCC_RATES','NVOCC_DOCUMENTS','CARRIER_SPACE_CONTROL','FORWARDING_PORTAL','FORWARDING_QUOTES','FORWARDING_BOOKINGS','FORWARDING_DIRECT_COLOAD_CROSS_TRADE','KYC_APPROVAL','RATE_PROVIDER_ADMIN','JOB_MODEL_CONVERT','FINANCE','GL_VIEW','GL_CREATE','GL_EDIT','GL_DISABLE','GL_POST','JOURNAL_CREATE','JOURNAL_APPROVE','PERIOD_CLOSE','ACCOUNT_MAPPING_MANAGE','COST_PROFIT_CENTER_MANAGE','TAX_CURRENCY_MANAGE','FINANCE_CONFIG_ADMIN'];
...
export default function AdministrationPage(){
  ...
  return <WorkspaceShell title="Administration" subtitle="Branches, managed user access, scopes and integration monitoring" active="/administration" actions={<button className="btn" onClick={()=>void load()}>Refresh</button>}>
    ...
    {tab==='USERS'&&<>
      ...
      <div style={{marginTop:12}}><span style={labelStyle}>User Rights</span><div className="sub" style={{marginBottom:6}}>GL and Finance & Accounting Setup rights are explicit. FINANCE, OPS and AGENT users do not receive them automatically; Global Admin assigns them only to selected users.</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:6}}>
